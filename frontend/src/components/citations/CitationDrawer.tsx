      ...
      <div className="fixed inset-y-0 right-0 flex max-w-full pl-6">
        <div className="flex w-screen max-w-lg flex-col bg-[#0e1424] text-white shadow-2xl">
          <div className="flex items-center justify-between border-b border-white/10 p-6">
            <div><h2 id="citation-title" className="flex items-center gap-2 text-lg font-bold"><Quotes className="text-cyan-400" />{title}</h2><p className="text-xs text-slate-400">{subtitle}</p></div>
          ...
          <div className="flex-1 space-y-4 overflow-y-auto p-6">
            ...
            {!citations.length && <p className="rounded-xl border border-dashed border-white/10 p-6 text-center text-xs text-slate-400">This response has no citations.</p>}
            ...
            {showRaw && <pre className="max-h-64 overflow-auto rounded-xl border border-white/10 bg-black/50 p-4 text-[11px] text-cyan-200">{JSON.stringify(rawPayload, null, 2)}</pre>}
          </div>
          <div className="flex justify-between border-t border-white/10 p-6 text-xs text-slate-400"><span>Exact source coordinates retained</span><button type="button" onClick={() => void copy(JSON.stringify(rawPayload, null, 2), 'all')} className="flex items-center gap-1.5 rounded-lg bg-white/10 px-3 py-1.5 text-white">{copied === 'all' ? <Check className="text-emerald-400" /> : <Copy />}Copy JSON</button></div>
        </div>
      </div>
    </div>
  );
