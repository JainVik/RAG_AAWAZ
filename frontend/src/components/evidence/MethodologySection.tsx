  ...
  return (
    <div className="refractive-glass-card refractive-glass-card-primary overflow-hidden">
      <button
        ...
        className="w-full p-4 sm:p-6 md:p-8 flex items-center justify-between text-left hover:bg-black/[0.02] dark:hover:bg-white/[0.04] transition-colors cursor-pointer gap-3"
      ...
      >
        <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
          <div className="p-2 sm:p-2.5 rounded-xl bg-blue-500/10 dark:bg-blue-500/15 border border-blue-400/30 text-blue-600 dark:text-blue-300 shrink-0">
            <BookOpen size={20} weight="bold" className="sm:w-[22px] sm:h-[22px]" />
          </div>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 text-[11px] sm:text-xs font-semibold text-black dark:text-slate-400 shrink-0">
          <span className="hidden sm:inline">{isOpen ? 'Collapse' : 'Expand details'}</span>
          <CaretDown
            size={16}
      ...
      {isOpen && (
        <div className="p-4 sm:p-6 md:p-8 pt-0 border-t border-black/10 dark:border-white/10 space-y-4 sm:space-y-6 text-xs text-black dark:text-slate-300 leading-relaxed font-sans">
          {/* Methodological Statements */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
            <div className="p-3 sm:p-4 bg-black/[0.03] dark:bg-white/[0.04] border border-black/10 dark:border-white/10 rounded-xl space-y-1.5 transition-all hover:bg-black/[0.06] dark:hover:bg-white/[0.07]">
              ...
              <p className="text-[11px] sm:text-xs text-black dark:text-slate-400">
            ...
            <div className="p-3 sm:p-4 bg-black/[0.03] dark:bg-white/[0.04] border border-black/10 dark:border-white/10 rounded-xl space-y-1.5 transition-all hover:bg-black/[0.06] dark:hover:bg-white/[0.07]">
              <span className="font-bold text-black dark:text-white flex items-center gap-1.5 text-xs">
                <LockKey size={16} className="text-blue-600 dark:text-blue-400 shrink-0" />
          ...
          <div className="space-y-2">
            <span className="font-bold text-black dark:text-white uppercase tracking-wider text-[10px] sm:text-[11px] block">
              Cryptographic Artifact Hashes (SHA256)
            </span>
            <div className="p-3 sm:p-4 bg-black/[0.03] dark:bg-black/40 border border-black/10 dark:border-white/10 rounded-xl space-y-2 font-mono text-[10px] sm:text-[11px] backdrop-blur-sm">
              {Object.entries(provenance.artifact_hashes).map(([file, hash]) => (
                <div key={file} className="flex flex-col sm:flex-row sm:items-center justify-between gap-0.5 sm:gap-2 py-1 border-b border-black/5 dark:border-white/5 last:border-0">
                  <span className="text-blue-600 dark:text-blue-300 font-semibold truncate">{file}</span>
                  <span className="text-black dark:text-slate-400 break-all sm:truncate max-w-full sm:max-w-md" title={String(hash)}>
                    {String(hash)}
                  </span>
                </div>
              ))}
