  ...
  return (
    <article className="refractive-glass-card refractive-glass-card-primary space-y-5 p-6 sm:p-8">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 pb-4">
        ...
        <span className="rounded-full border border-blue-400/30 bg-blue-500/15 px-3 py-1 font-mono text-xs font-semibold text-blue-200">
          Operational · non-qualifying
        </span>
      </div>

      <div className="glass-inner-box flex gap-2 text-[10px] leading-relaxed text-slate-300">
      ...
      {metrics?.latency_ms ? (
          ...
          <div className="flex flex-wrap items-end justify-between gap-2">
            <div>
              <p className="flex items-center gap-1.5 text-xs font-bold text-slate-200">
                <ChartLine size={15} className="text-blue-400" /> Overall primary response
              </p>
              <p className="mt-1 text-[10px] text-slate-400">{metrics.latency_sample_count} timed responses from {metrics.requests_total} process requests</p>
            </div>
          </div>
          ...
          {rows.length > 0 && (
            <details className="glass-inner-box overflow-x-auto">
              ...
              <div className="mt-3 min-w-[620px]">
                <div className="grid grid-cols-[minmax(9rem,1.5fr)_repeat(4,minmax(4.2rem,0.7fr))] gap-2 border-t border-white/10 pt-2 text-[9px] font-bold uppercase tracking-wider text-slate-400">
                ...
                <p className="border-t border-white/10 pt-2 text-[9px] text-slate-400">
          ...
          {metrics.groq_synthesis.latency_ms && (
            <div className="rounded-xl border border-violet-400/20 bg-violet-500/10 p-4">
              <div className="mb-3 flex items-center justify-between gap-2">
                ...
                <span className="text-[9px] text-slate-400">{metrics.groq_synthesis.latency_sample_count} samples · measured separately</span>
              </div>
        ...
        <div className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/[0.04] p-5 text-sm text-slate-300">
          <ClockCounterClockwise size={20} className="shrink-0 text-slate-400" />
          <div>
            <p className="font-semibold text-white">Live timing is not available yet</p>
            <p className="mt-1 text-xs text-slate-400">{error ?? 'Run a text or voice query to create process-local timing samples.'}</p>
          </div>
        </div>
      )}
    </article>
