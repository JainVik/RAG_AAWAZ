    ...
    <article className="refractive-glass-card p-6 transition-all hover:border-blue-500/30">
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-black/10 dark:border-white/10 pb-4">
          ...
          <div className="flex items-center gap-2">
            <span className="text-[11px] font-mono text-black dark:text-slate-400">Scope:</span>
            <span className="px-2.5 py-1 rounded-lg text-xs font-mono font-bold bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-blue-600 dark:text-blue-300">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <div className="p-3.5 bg-black/[0.03] dark:bg-white/5 border border-black/10 dark:border-white/8 rounded-xl space-y-1">
            ...
            <span className="text-[10px] text-black dark:text-slate-400 block">Strict type match</span>
          </div>
          ...
          <div className="p-3.5 bg-black/[0.03] dark:bg-white/5 border border-black/10 dark:border-white/8 rounded-xl space-y-1">
            ...
            <span className="text-black dark:text-white font-mono font-bold text-sm block">
              {audit.malformed_row_count} / {audit.duplicate_query_count}
            </span>
            <span className={`text-[10px] block font-medium ${defectCount === 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-600 dark:text-amber-300'}`}>
          ...
          <div className="p-3.5 bg-black/[0.03] dark:bg-white/5 border border-black/10 dark:border-white/8 rounded-xl space-y-1">
            ...
            <span className="text-[10px] text-black dark:text-slate-400 block">Across {audit.audited_row_count} audited queries</span>
          </div>
          ...
          <div className="p-3.5 bg-black/[0.03] dark:bg-white/5 border border-black/10 dark:border-white/8 rounded-xl space-y-1">
            ...
            <span className="text-blue-600 dark:text-blue-400 font-mono font-bold text-sm block">
              {audit.selected_passage_ratio === null ? 'Not measured' : `${(audit.selected_passage_ratio * 100).toFixed(1)}%`}
            </span>
            <span className="text-[10px] text-black dark:text-slate-400 block">Selected candidate ratio</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div className="p-3.5 bg-black/[0.03] dark:bg-white/5 border border-black/10 dark:border-white/8 rounded-xl space-y-2">
            <div className="flex items-center gap-1.5 text-xs font-bold text-black dark:text-slate-300">
              <ChartPieSlice size={15} className="text-blue-600 dark:text-blue-400" />
              <span>Query Type Distribution</span>
            </div>
            <div className="grid grid-cols-3 gap-2 text-center pt-1">
              {Object.entries(audit.query_type_distribution || {}).map(([type, count]) => (
                <div key={type} className="p-2 bg-black/5 dark:bg-white/5 rounded-lg border border-black/5 dark:border-white/5">
                  <span className="text-xs font-bold text-black dark:text-white font-mono">{count}</span>
                  <span className="text-[10px] text-black dark:text-slate-400 block mt-0.5">{type}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="p-3.5 bg-black/[0.03] dark:bg-white/5 border border-black/10 dark:border-white/8 rounded-xl space-y-1.5 text-xs">
            <div className="flex items-center gap-1.5 text-xs font-bold text-black dark:text-slate-300 mb-1">
              <ListNumbers size={15} className="text-blue-600 dark:text-blue-400" />
              <span>Provenance &amp; Split</span>
            </div>
            <div className="flex justify-between text-[11px]">
              <span className="text-black dark:text-slate-400">Dataset ID:</span>
              <span className="font-mono text-black dark:text-slate-200">{audit.dataset_id}</span>
            </div>
            <div className="flex justify-between text-[11px]">
              <span className="text-black dark:text-slate-400">Source Split:</span>
              <span className="font-mono text-black dark:text-slate-200">{audit.source_split} ({audit.target_language})</span>
            </div>
            <div className="flex justify-between text-[11px]">
              <span className="text-black dark:text-slate-400">Revision:</span>
              <span className="font-mono text-black dark:text-slate-200">{audit.revision}</span>
            </div>
          </div>
        </div>
        ...
        <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 text-xs text-amber-800 dark:text-amber-300 flex items-start gap-2.5">
          <ShieldWarning size={18} className="shrink-0 mt-0.5 text-amber-600 dark:text-amber-400" />
