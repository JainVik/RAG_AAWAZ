...
export const LiveTranscriptCard: React.FC<LiveTranscriptCardProps> = ({
      ...
      <div className="refractive-glass-card refractive-glass-card-primary p-5 sm:p-6 space-y-4">
        <div className="flex items-center justify-between gap-2 border-b border-black/10 dark:border-white/10 pb-3">
          <div className="flex items-center gap-2 text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">
            <ChatCenteredText size={16} className="text-blue-600 dark:text-blue-400" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-1.5 text-xs font-bold text-blue-700 dark:text-blue-300 px-1">
            ...
            <div className="space-y-1.5">
              {SAMPLE_QUESTIONS.filter((q) => q.code === 'hi').map((p, idx) => (
                ...
                >
                  <p className="text-xs text-black dark:text-slate-200 group-hover:text-blue-600 dark:group-hover:text-blue-300 leading-relaxed font-medium">
                    {p.text}
                  </p>
                </button>
              ))}
          ...
          {/* English Section */}
          <div className="space-y-2">
            <div className="flex items-center gap-1.5 text-xs font-bold text-emerald-700 dark:text-emerald-400 px-1">
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2 border-b border-black/10 dark:border-white/10 pb-3">
        ...
        <div className="flex items-center gap-2">
          {detectedLanguage && detectedLanguage !== 'unknown' && (
            <span className="px-2 py-0.5 rounded bg-white/80 dark:bg-black/30 border border-black/10 dark:border-white/10 text-[10px] font-mono font-bold text-black dark:text-white uppercase">
              {getLanguageDisplayLabel(detectedLanguage)}
            </span>
          )}

          <div
            className="hidden sm:inline-flex items-center gap-1 text-[11px] text-black dark:text-slate-400"
      ...
      <div className="relative">
        <p
          className={`text-base sm:text-lg font-medium leading-relaxed ${
            isPartial ? 'text-black dark:text-slate-300 italic' : 'text-black dark:text-white'
          }`}
        >
      ...
      <div className="sm:hidden text-[10px] text-black dark:text-slate-500 pt-1">
