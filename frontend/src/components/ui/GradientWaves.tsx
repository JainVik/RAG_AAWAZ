...
void main() {
  ...
  if (uEnableMouse) {
    float yaw = (uMouse.x - 0.5) * uParallax * 0.8;
    float pitch = (uMouse.y - 0.5) * uParallax * 0.8;
    c = cos(yaw); s = sin(yaw);
    dir = mat3(c, 0.0, s, 0.0, 1.0, 0.0, -s, 0.0, c) * dir;
...
export const GradientWaves: React.FC<GradientWavesProps> = ({
  ...
  useEffect(() => {
    ...
    const onPointerMove = (e: MouseEvent | PointerEvent) => {
      ...
      targetMouse[0] = Math.max(0, Math.min(1, e.clientX / w));
      targetMouse[1] = Math.max(0, Math.min(1, 1.0 - e.clientY / h));
    };
